...
| 面板 | 显示什么 |
|---|---|
| **授权清单** | 每条授权的单笔上限、累计上限、到期时间与已用额度。**已撤销的那条会留在屏幕上并显示 `已撤销`** —— 一条凭空消失的行，比一条明显死掉的行，给用户的信息更少 |
| **一致性时间线** | 收据流；越权标红，被拒的那笔带着拦住它的策略，多笔支付按任务聚合（`N 笔 · 总额 · N 笔被拦`） |
| **链上锚定** | 本地锚定记录，并排对着链上的 `latestRoot` **和** `lastEpoch` —— 两条独立结论，因为"根对得上但 epoch 过期"正是记录漂移的样子 |
| **三态验证** | `verified` / `tampered` / `incomplete`，并列出产生该结论的具体问题 |
| **证据包** | 一个 zip：收据 + 锚定记录 + 逐张收据的包含证明 + JWKS + 一个你可以自己跑的验证脚本 |

账本类视图每 3 秒轮询一次，锚定卡片每 5 秒，所以 agent 花钱时收据会自动出现。但**操作不会等着轮询**：按下熔断，所有视图当场被拉前刷新。
...
components/
  mandate-list        授权清单：额度进度、撤销状态
  timeline            收据流：任务聚合、越权标记
  kill-switch         一键撤销全部授权
  anchor-status       链上锚定 + 两条一致性结论（wagmi）
